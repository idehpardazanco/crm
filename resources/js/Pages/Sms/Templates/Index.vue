...
const totalTemplates = computed(() => {
    return props.templates?.total
        ?? props.templates?.data?.length
        ?? 0
})

const activeTemplates = computed(() => {
    return (props.templates?.data ?? []).filter(
...
function confirmDelete() {
    ...
    router.delete(
        `/sms/templates/${selectedTemplate.value.id}`,
        {
            preserveScroll: true,
...
<template>
    ...
    >
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="mt-2 text-sm text-slate-500">
                        مدیریت متن‌های آماده برای ارسال سریع
                    </p>
                </div>
            </div>
        ...
        >
            ...
            >
                <input
                    v-model="search"
                    type="search"
                    placeholder="جستجوی عنوان یا نوع قالب"
                ...
                <table class="min-w-[900px]">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td>
                                <code
                                    class="rounded-lg bg-slate-100 px-3 py-1.5 text-xs text-slate-600"
                                >
                                    {{ item.type || '-' }}
                                </code>
                            </td>
                            ...
                            <td>
                                <div
                                    class="max-w-sm truncate text-sm text-slate-500"
                                >
                                    {{ item.body || '-' }}
                                </div>
                            </td>
                        ...
                        <tr v-if="!templates.data?.length">
                            ...
                            >
                                ...
                                >
                                    قالب جدید بسازید یا عبارت جستجو را تغییر
                                    دهید.
                                </span>
                            </td>
                        </tr>
        ...
        >
            <div>
                صفحه
                <strong>{{ templates.current_page ?? 1 }}</strong>
                از
                <strong>{{ templates.last_page ?? 1 }}</strong>
            </div>
        ...
        <Teleport to="body">
            <div
                ...
                dir="rtl"
                @click.self="!deleting && (selectedTemplate = null)"
            >
                <div
